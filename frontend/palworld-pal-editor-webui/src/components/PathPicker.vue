...
<template>
  <div
    class="modal-overlay"
    v-if="palStore.SHOW_FILE_PICKER"
    @click.self="abort">
    <div class="popup">
      ...
      <div class="currentPath">
        ...
        <IconButton
          icon="➡️"
          @click="
            palStore.update_picker_result(palStore.PAL_FILE_PICKER_PATH)
          " />
      </div>

      <ul ref="scrollElement">
        <li
          v-for="([key, value], index) of sortedPathChildren"
          :key="index"
          :isdir="value.isDir"
          @click="
            () => {
              if (value.isDir) palStore.update_picker_result(key);
            }
          "
          :fullpath="key">
      ...
      <BarButton
        ...
        content="OK"
        :disabled="!palStore.IS_PAL_SAVE_PATH" />
    </div>
  </div>
</template>
